...
void* command_center () {
	...
	return;	//Return from command_center.
}
...
void* solder () {
	...
	if (pthread_mutex_lock(&mutex_solders) != 0) {	//Check if pthread_mutex_lock fail and if so print error massage and force game_over().
		printf("Failed to lock mutex: solders!\n");
		game_over();
		return;
	}
	m++;	//Increment m.
	if (pthread_mutex_unlock(&mutex_solders) != 0) {	//Check if pthread_mutex_unlock fail and if so print error massage and force game_over().
		printf("Failed to unlock mutex: solders!\n");
		game_over();
		return;
	}	
	printf("You wanna piece of me, boy?\n");	//Indicate that new solder was successfuly created.
	return;
}

//--------------------------------------------
// FUNCTION: main
// To simulate the main command center initialize all mutex (except mutex_command_centers_minerals[]), join all hreads with main and destory all mutex.
// PARAMETERS:
// None.
//----------------------------------------------
int main () {
	...
	if (pthread_mutex_init(&mutex_solders, NULL) != 0) {	//Check if pthread_mutex_init fail and if so print error massage and force game_over().
	...
	while(m < 20) {
		...
		if ((command == 'm') || (command == 's') || (command == 'c')) {
			...
			if (command == 'm') {	//Check if written command is m and if so check is it possible to create new solder.
				if ((all_minerals >= 50) && (m < 20)) {	//Check if there are enough minerals and if m is valid and if so decrement all_minerals with 50.
					...
					if (pthread_create(&solders[m], NULL, solder, NULL) != 0) {	//Check if pthread_create fail and if so print error massage and force game_over().
		...
		if ((all_minerals == 0) && (map_minerals == 0)) {	//IF there is no way to create 20 solders assign ~ to command.
			command = '~';
			break;
		}
	}
	...
	for(i = 0; i < k;i++) {	//Loop k times and join all command_centers with main.
	...
	for(i = 0; i < m;i++) {	//Loop m times and join all solders with main.
		if (pthread_join(solders[i], NULL) != 0 ) {	//Check if pthread_join fail and if so print error massage and force game_over().
